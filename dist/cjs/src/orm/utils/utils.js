...
exports.dataObjectIdToString = exports.isStringObjectID = exports.isObjectID = exports.getEntityProperties = void 0;
const mongodb_1 = require("mongodb");
const orm_interfaces_1 = require("../interfaces/orm.interfaces");
function getEntityProperties(entity, repositoryOptions, EntityClass) {
    ...
    referenceEntities.forEach((ref) => {
        if (ref.refersToKey && (ref.type === orm_interfaces_1.RELATION_TYPES.ONE_TO_ONE) && !uniqueKeys.find((uni) => uni.key === ref.key)) {
            uniqueKeys.push({
                key: ref.key
            });
        }
        if (ref.refersToKey && !ref.type && ref.isArray) {
            ref.type = orm_interfaces_1.RELATION_TYPES.MANY_TO_MANY;
        }
    });
...
exports.isStringObjectID = isStringObjectID;
const dataObjectIdToString = (data, referenceEntities = []) => {
    var _a, _b, _c, _d;
    if (!data) {
        return;
    }
    ...
    if (Array.isArray(data)) {
        for (let i = 0; i < data.length; i++) {
            if (isObjectID(data[i])) {
                ...
                (0, exports.dataObjectIdToString)(data[i], referenceEntities);
            }
        }
    }
            ...
            if (keyRefEntity && Array.isArray(data[currentKey])) {
                for (let j = 0; j < data[currentKey].length; j++) {
                    if (isObjectID(data[currentKey][j])) {
                        ...
                        (0, exports.dataObjectIdToString)(data[currentKey][j], keyRefEntity.referenceEntities);
                    }
                }
            }
                ...
                if (isObjectID(data[currentKey])) {
                    ...
                    (0, exports.dataObjectIdToString)(data[currentKey], keyRefEntity.referenceEntities);
                }
            }
        }
        delete data._id;
...
exports.dataObjectIdToString = dataObjectIdToString;
